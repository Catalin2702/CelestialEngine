 ...
 * @brief Event triggered when the application window gains or loses input focus
 * @details Generated by the windowing system when the window becomes (or stops being) the key window.
 *			Carries a flag indicating the new focus state. Input-related subscribers (e.g. the engine
...
public:
	/**
	 * @brief Gets the new focus state of the window
	 * @return bool True if the window gained focus, false if it lost it
	 */
	[[nodiscard]] bool IsFocused() const { return _focused; }

	/**
	 * @brief Converts the event to a string representation
	 * @return std::string String containing the focus state
	 */
	[[nodiscard]] std::string ToString() const override;
